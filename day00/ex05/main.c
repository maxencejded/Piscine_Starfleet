...
int main(void)
{
	...
	if (!(dict = dictInit(lenWords)))
	{
		printf("Can't malloc the dictionnary\n");
		return (1);
	}
	for (int i = 0 ; i < lenWords ; i += 1)
	{
		if (!dictInsert(dict, words[i], i + 1))
		{
			printf("Can't add key \"%s\" in the dictionnary\n", words[i]);
			return (1);
		}
	}
	printf("%s\n", compress(book, dict));
